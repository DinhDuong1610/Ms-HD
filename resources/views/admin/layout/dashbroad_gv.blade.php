...
@section('main')
    ...
    <ul class="info-blocks">
        ...
        <a href="{{ url('giaovien/loaicauhoi/dsloai') }}">
            <li class="bg-danger">
                ...
                <a href="{{ url('giaovien/loaicauhoi/dsloai') }}"><a href="#"><i
                            class="fas fa-chalkboard-teacher"></i> </a>
                    <span class="bottom-info bg-primary"></span>
                    {{-- <span class="bottom-info bg-primary">{{ count($quesion_type) }}</span> --}}
    ...
    {{-- <script type="text/javascript">
        Highcharts.chart('container', {
            ...
            series: [{
                name: 'Điểm Liệt',
                data: [{{ count($diemlietmh1) }}, {{ count($diemlietmh2) }}, {{ count($diemlietmh3) }},
                    {{ count($diemlietmh4) }}, {{ count($diemlietmh5) }}, {{ count($diemlietmh6) }},
                    {{ count($diemlietmh7) }}, {{ count($diemlietmh8) }}, {{ count($diemlietmh9) }},
                    {{ count($diemlietmh10) }}, {{ count($diemlietmh11) }}
                ]

            }, {
                name: 'Điểm TB',
                data: [{{ count($diemtbmh1) }}, {{ count($diemtbmh2) }}, {{ count($diemtbmh3) }},
                    {{ count($diemtbmh4) }}, {{ count($diemtbmh5) }}, {{ count($diemtbmh6) }},
                    {{ count($diemtbmh7) }}, {{ count($diemtbmh8) }}, {{ count($diemtbmh9) }},
                    {{ count($diemtbmh10) }}, {{ count($diemtbmh11) }}
                ]

            }, {
                name: 'Điểm Khá',
                data: [{{ count($diemkhamh1) }}, {{ count($diemkhamh2) }}, {{ count($diemkhamh3) }},
                    {{ count($diemkhamh4) }}, {{ count($diemkhamh5) }}, {{ count($diemkhamh6) }},
                    {{ count($diemkhamh7) }}, {{ count($diemkhamh8) }}, {{ count($diemkhamh9) }},
                    {{ count($diemkhamh10) }}, {{ count($diemkhamh11) }}
                ]

            }, {
                name: 'Điểm Giỏi',
                data: [{{ count($diemgioimh1) }}, {{ count($diemgioimh2) }}, {{ count($diemgioimh3) }},
                    {{ count($diemgioimh4) }}, {{ count($diemgioimh5) }}, {{ count($diemgioimh6) }},
                    {{ count($diemgioimh7) }}, {{ count($diemgioimh8) }}, {{ count($diemgioimh9) }},
                    {{ count($diemgioimh10) }}, {{ count($diemgioimh11) }}
                ]
